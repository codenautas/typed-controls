...
casper.test.begin("Test checkbox", function(test) {
    casper.start(testUrl, function() {
        ...
        sendKey(keys.Space);
        bool1 = getInfo(boolN);
        test.assertEquals(bool1.value , false, "space sets to false");
        sendKey(keys.Delete);
        bool1 = getInfo(boolN);
        test.assert(! bool1.value, "delete sets null");
        sendKey(keys.Space);
        bool1 = getInfo(boolN);
        test.assertEquals(bool1.value, false, "space sets to false (2)");
        sendKey(keys.Backspace);
        bool1 = getInfo(boolN);
        test.assert(! bool1.value, "backspace sets null");
        sendKey(keys.Space);
        bool1 = getInfo(boolN);
        test.assertEquals(bool1.value , false, "space sets to false (3)");
        sendKey(keys.Minus);
        bool1 = getInfo(boolN);
        test.assert(! bool1.value, "minus sets null");
        sendKey(keys.Space);
        bool1 = getInfo(boolN);
        test.assertEquals(bool1.value , false, "space sets to false (4)");
        sendKey(keys.Period);
        bool1 = getInfo(boolN);
