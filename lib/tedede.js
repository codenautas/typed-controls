...
function CreadorBooleanRadiobuttonProvisorio(id){
    return CreadorOptionsRadiobuttonProvisorio(id,{true:'Sí', false:'No'});
}

function CreadorOptionsRadiobuttonProvisorio(id, options){
    
    id=iiiiiiid++;
    id=('tedede'+id).replace('.','').substr(0,12);
...
BestTypes["number"] = {
    adapters:{
        validateTypedData: function validateTypedDataNumber(typedValue){
            ...
            /*if(typedValue%1!=0){
                throw new Error('blah '+typeof typedValue);
            }*/
        },
...
/*BestTypes["enum"] = {
    adapters:{
        ...
        getTypedValue: function getTypedValueEnum(){
            var tededeOptionGroup=this.getAttribute("tedede-option-group");
            if(tededeOptionGroup && this.tededeTypeInfo.options){
                var typedValue = null;
