...
function CreadorBooleanRadiobuttonProvisorio(id){
    return CreadorOptionsRadiobuttonProvisorio(id,{true:{label:'Sí'}, false:{label:'No'}});
}

function CreadorOptionsRadiobuttonProvisorio(id, options){
    id=iiiiiiid++;
    id=('tedede'+id);
    return html.div({id:id, "tedede-option-group": id},[html.div(
        Object.keys(options).map(function(value){
            return html.div([
                html.input({type:'radio', name:id, value:value, id:id+'-'+value }), 
                html.label({"for":id+'-'+value },options[value].label), 
                html.br()
            ]);
        })
    )]);
...
BestTypes["enum"] = {
    adapters:{
        validateTypedData: function validateTypedDataBoolean(typedValue){
            if(typedValue!=null && !this.tededeTypeInfo.options[typedValue]){
        ...
        getTypedValue: function getTypedValueEnum(){
            ...
            if(tededeOptionGroup){
                var typedValue = null;
                Object.keys(this.tededeTypeInfo.options).forEach(function(value){
                    if(document.getElementById(tededeOptionGroup+'-'+value).checked){
                        typedValue = value;
                    }
                });
    ...
    domFixtures:[
        {creatorFunction:CreadorOptionsRadiobuttonProvisorio, attributes:{}},
    ]
};
...
Tedede.adaptElement = function adaptElement(domElement, typeInfo){
    if(typeof typeInfo === 'string'){
        ...
        if(typeInfo.typeName==='boolean'){
            typeInfo.options={true:'sí', false:'no'}
        }
    }
    ...
    if(tededeOptionGroup){
        ...
        Object.keys(typeInfo.options).forEach(function(value){
...
Tedede.createFromFixture = function createFromFixture(fixture, options){
    var thisModule = this;
    if(fixture.creatorFunction){
        return fixture.creatorFunction(Math.random(), options);
    }else{
        return html[fixture.tagName].call(html,fixture.attributes,(fixture.content||[]).map(function(subFixture){
            return thisModule.createFromFixture(subFixture, options);
        }));
    }
}
