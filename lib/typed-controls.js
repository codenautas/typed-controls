...
TypedControls.enablePasteCorrections=true;
...
BestControls.div = {
    ...
    listeners:{
        ...
        'paste': function(event){
            if (TypedControls.enablePasteCorrections){
                let paste = (event.clipboardData || window.clipboardData).getData('text');
                paste = paste.replace(/\u00A0/g,' ');
                var element = event.target;
                if(element && (element.tagName == "INPUT" || element.tagName == "TEXTAREA")){
                    var start = element.selectionStart;
                    element.setRangeText(paste);
                    var pos = (start||0) + paste.length
                    element.setSelectionRange(pos, pos);
                }else{
                    const selection = window.getSelection();
                    if (!selection.rangeCount) return false;
                    console.log(selection.rangeCount);
                    for(var i=0; i<selection.rangeCount; i++) console.log('e',selection.getRangeAt(i))
...
TypedControls.adaptElement = function adaptElement(domElement, typerOrTypeInfo){
    ...
    if(typedControlsOptionGroup){
        ...
        domElement.disable=function(disabled){
            ...
            if(inputElement && inputElement !== domElement){
        ...
        if(expander){
            ...
            var reubicate = function reubicate(){
                if(domElement.lupaFija || (document.activeElement === domElement || domElement.disabled) && editable(typer, expander, domElement)){
            ...
            domElement.ponerLupa = function ponerLupa(fija){
                ...
                domElement.lupaFija = fija;
            }
            var quitarLupa = function quitarLupa(domElement){
                if(domElement.lupa && !domElement.lupaFija){
                    clearInterval(domElement.lupa.timerReubicate);
            ...
            var displayExpander = function displayExpander(event, opts){
                if(editable(typer, expander, domElement)){
                    ...
                    var dialogFinally = function dialogFinally(hasUpdated){
                        domElement.updatingSuspended=false;
                        domElement.focus();
                        if(!domElement.disabled && !(hasUpdated instanceof Error)){
        ...
        domElement.validateWhileInput = function validateWhileInput(){
            ...
            if(plainValue==null){
                ...
                var typer = this.controledType; // eslint-disable-line no-redeclare
