    ...
    adapters:{
        ...
        getTypedValue: function getTypedValueNumber(){
            ...
            if(value===null){
                ...
                value = typeStore.type.bigint.fromString(value);
            }
            return value;
        },
...
BestTypes["FROM:type-store"] = {
    adapters:{
        getTypedValue: function getTypedValueDate(){
            ...
            return this.fromString(plainValue);
        },
        setValidatedTypedValue: function setValidatedTypedValuePrettyDate(typedValue){
             if(typedValue==null){
                this.setPlainValue(typedValue);
...
TypedControls.adaptElement = function adaptElement(domElement, typeInfo){
    ...
    if(typedControlsOptionGroup){
        ...
        if(domElement.getAttribute("typed-controls-direct-input")){
        ...
        if(expander){
            var reubicate = function reubicate(){
                if(document.activeElement === domElement){
                    ...
                    domElement.lupa.style.display='none';
                }
            }
            ...
            domElement.lupa = html.img({src:'lupa.png', class:'lupa-typed-controls', style:'position:absolute; display:none'}).create();
            document.body.appendChild(domElement.lupa);
            domElement.lupa.addEventListener('load', reubicate);
            domElement.lupa.addEventListener('click',function(){
                displayExpander();
            });
            domElement.addEventListener('focus', function(event){
                reubicate();
            });
            ...
            var timerReubicate = setInterval(reubicate,500);
            domElement.addEventListener('DOMNodeRemoved', function(){
                if(timerReubicate){
                    cancelInterval(timerReubicate);
                    timerReubicate=null;
...
for(var typeName in typeStore.type){
    if(!(typeName in BestTypes)){
        if(!typeStore.type[typeName].typedControlName){
            throw new Error('typed-controls: lack of typedControlName in type-store for '+typeName);
        }
        BestTypes[typeName] = changing(BestTypes[typeStore.type[typeName].typedControlName],{});
        ['validateTypedData', 'fromString', 'toPlainString', 'toJsHtml'].forEach(function(adapterName){
            if(adapterName in typeStore.type[typeName]){
                BestTypes[typeName].adapters[adapterName] = typeStore.type[typeName][adapterName];
            }
        });
    }
}
