...
TypedControls.adaptElement = function adaptElement(domElement, typerOrTypeInfo){
    ...
    if(typedControlsOptionGroup){
        ...
        if(inputElement){
            ...
            inputElement.addEventListener('change',function(){
                ...
                if(value && domElement.optionControl[value]){
                    ...
                    inputElement.value = domElement.getTypedValue();
                }*/
            });
        ...
        if(expander){
            ...
            domElement.addEventListener('keydown', function(event){
                var domElement = this;
                if(event.keyCode == 119 && !event.shiftKey && !event.ctrlKey && !event.altKey
    ...
    domElement.setTypedValue = function setTypedValue(typedValue, fromUserInteraction){
        ...
        if(fromUserInteraction){
            if(!bestGlobals.sameValue(this.previousValue, typedValue)){
                this.dispatchEvent(new CustomEvent('update'));
            }
        }
        this.previousValue=typedValue;
    };
};
