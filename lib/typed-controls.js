...
TypedControls.adaptElement = function adaptElement(domElement, typer){
    // typeInfo = TypedControls.completeTypeInfo(typeInfo);
    if(typeof typer == "text"){
        throw new Error("obsolete typeName in adaptElement "+typer);
    }
    if(!(typer instanceof TypeStore.TypeBase)){
        typer = TypeStore.typerFrom(typer);
    }
    var typeName = typer.typeName;
    var typedControlsOptionGroup=domElement.getAttribute("typed-controls-option-group");
    domElement.controledTypeInfo = typer;
    ...
    if(typedControlsOptionGroup){
        ...
        typer.options.forEach(function(option){
        ...
        if(radioChilds.length!=typer.options.length){
...
likeAr(TypeStore.type).forEach(function(typeDef, typeName){
    // if(!(typeName in BestControls) || ){
        if(typeof typeDef !== 'function'){
            console.log('xxxxxxxxxxxxx',typeName, typeDef);
        }
