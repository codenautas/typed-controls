...
BestControls.div = {
    ...
    listeners:{
        ...
        'keypress': function onKeyPressListenerForDiv(event){
            ...
            if(event.which>=32){
                var typer = this.controledType;
                if(typer.rejectedChar(String.fromCharCode(event.which))){
...
TypedControls.adaptElement = function adaptElement(domElement, typerOrTypeInfo){
    ...
    if(typedControlsOptionGroup){
        ...
        var expander = TypedControls.Expanders.find(function(expander){
            ...
            return (typer.typeInfo.editable == null || typer.typeInfo.editable || expander.whenReadOnly) && expander.whenType(domElement);
        });
        if(expander){
            domElement.lupa=null;
            var reubicate = function reubicate(){
                if(document.activeElement === domElement || domElement.disabled){
            ...
            var displayExpander = function displayExpander(event){
            ...
            ['focus','click'].forEach(function(eventName){
                if(TypedControls.showLupa && TypedControls.showOn[eventName]){
                    domElement.addEventListener(eventName, function(){
                        if(!domElement.lupa){
                            domElement.lupa = html.img({src:TypedControls.path.img+(expander.img||'lookdown.png'), class:'img-lupa', style:'position:absolute; display:none'}).create();
