...
BestControls.div = {
    adapters:{
        getTypedValue: function getTypedValueDate(){
            ...
            return typer.fromLocalString(plainValue);
        },
...
TypedControls.adaptElement = function adaptElement(domElement, typerOrTypeInfo){
    ...
    if(typedControlsOptionGroup){
        ...
        /*
        if(TypedControls.ENABLE_SIGNALS){
            var addSpecialOptionIfNeeded = function addSpecialOptionIfNeeded(optionName, defaultValue, label){
                var valueSpecial;
                if(optionName in typer.typeInfo){
                    valueSpecial=typer.typeInfo.valueNoData;
                }else{
                    valueSpecial=defaultValue;
                }
                var optionSpetial=typer.typeInfo.options.find(function(option){ return option.option == valueSpecial;});
                if(!optionSpetial){
                    optionSpetial = {option:valueSpecial, label}
                    typer.typeInfo.options.push()
                }
                if(!('hiddenWhenUnselected' in optionSpetial)){
                    optionSpetial.hiddenWhenUnselected=true;
                }
            };
            addSpecialOptionIfNeeded('valueNoData', TypedControls.VALUE_NO_DATA, TypedControls.messages.noData);
            addSpecialOptionIfNeeded('valueUnknownData', TypedControls.VALUE_UNKNOWN_DATA, TypedControls.messages.unknownData);
        }
        */
