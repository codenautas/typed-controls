...
TypedControls.adaptElement = function adaptElement(domElement, typerOrTypeInfo){
    ...
    domElement.setTypedValue = function setTypedValue(typedValue, fromUserInteraction){
        ...
        var setTypedValueFun = function setTypedValueFun(){
            ...
            if(TypedControls.ENABLE_SIGNALS){
                if(typedValue==TypedControls.SIGNAL_UNKNOWN_DATA || typedValue==TypedControls.VALUE_UNKNOWN_DATA || 
                    (self.controledType.typeInfo.valueUnknownData && self.controledType.typeInfo.valueUnknownData == typedValue)){
                    self.setAttribute('typed-controls-signal-unknown-data','1');
                }else{
                    self.removeAttribute('typed-controls-signal-unknown-data');
                }
                if(typedValue==TypedControls.SIGNAL_NO_DATA || typedValue==TypedControls.VALUE_NO_DATA ||
                    (self.controledType.typeInfo.valueNoData && self.controledType.typeInfo.valueNoData == typedValue)){
