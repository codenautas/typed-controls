...
BestTypes["FROM:type-store"] = {
    adapters:{
        getTypedValue: function getTypedValueDate(){
            var plainValue = this.getPlainValue();
            if(!this.fromString){
                console.log('xxxxxxxx sin fromString');
            }
            if(plainValue==null){
                return plainValue;
            }
        ...
        setValidatedTypedValue: function setValidatedTypedValuePrettyDate(typedValue){
             if(typedValue==null){
                ...
                this.setPlainValue(typedValue.join(';'));
                if(this.tagName!=='INPUT'){
                    this.innerHTML='';
                    var x=[];
                    ...
                    this.appendChild(html.span({class:'array'}, x).create());
                }
            }
        },
    },
...
for(var typeName in typeStore.type){
    if(!(typeName in BestTypes)){
        ...
        ['validateTypedData', 'fromString'].forEach(function(adapterName){
